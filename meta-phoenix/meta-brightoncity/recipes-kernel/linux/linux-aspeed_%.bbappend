...
do_patch:append() {
    ...
    if [ -r "${WORKDIR}/aspeed-bmc-phoenix-brightoncity.dts" ]; then
        cp ${WORKDIR}/aspeed-bmc-phoenix-brightoncity.dts \
            ${STAGING_KERNEL_DIR}/arch/${ARCH}/boot/dts
    fi
    ...
    if [ -r "${DEVTOOL_TEMPDIR}/oe-local-files/aspeed-bmc-phoenix-brightoncity.dts" ]; then
        cp ${DEVTOOL_TEMPDIR}/oe-local-files/aspeed-bmc-phoenix-brightoncity.dts \
            ${STAGING_KERNEL_DIR}/arch/${ARCH}/boot/dts
    fi
