...
SRC_URI += " \
        file://morocity.cfg \
        file://aspeed-bmc-phoenix-morocity.dts \
        file://aspeed-g5.dtsi \
...
do_patch:append() {
    ...
    if [ -r "${WORKDIR}/aspeed-bmc-phoenix-morocity.dts" ]; then
        cp ${WORKDIR}/aspeed-bmc-phoenix-morocity.dts \
            ${STAGING_KERNEL_DIR}/arch/${ARCH}/boot/dts
    fi
    ...
    if [ -r "${DEVTOOL_TEMPDIR}/oe-local-files/aspeed-bmc-phoenix-morocity.dts" ]; then
        cp ${DEVTOOL_TEMPDIR}/oe-local-files/aspeed-bmc-phoenix-morocity.dts \
            ${STAGING_KERNEL_DIR}/arch/${ARCH}/boot/dts
    fi
