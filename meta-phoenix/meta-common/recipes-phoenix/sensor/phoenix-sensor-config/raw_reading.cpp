...
int32_t get_bmc_sel_state (double *reading)
{    
    if (api_is_bmc_sel_empty() == true)
    {
        *reading = BIT(2); // SensorType 10h, offset 02h: Log Area Reset/Cleared
    } else {
        //*reading &= ~(BIT(2));
        *reading = 0;
    }
...
void update_cpu_present(int cpu_index)
{
    ...
    if (ret != 0) {
        cpu_present[cpu_index] = false;
    } else {
        cpu_present[cpu_index] = true;
    }
}
...
int32_t get_system_crash (double *reading)
{
    ...
    update_cpu_present(CPU_ID2);

    if ((cpu_present[CPU_ID1] == true && is_cpu_err(CPU_ID1) == true) ||
        (cpu_present[CPU_ID2] == true && is_cpu_err(CPU_ID2) == true)) {
