...
static int32_t get_current_power_state_from_state_manager_dbus(void)
{
    ...
    } else if (powerState == "xyz.openbmc_project.State.Chassis.PowerState.TransitioningToOff") {
        CurrentPowerState = POWER_STATE_TRANSITIONING_TO_OFF;
    } else if (powerState == "xyz.openbmc_project.State.Chassis.PowerState.TransitioningToOn") {
        CurrentPowerState = POWER_STATE_TRANSITIONING_TO_ON;
    } else {
        CurrentPowerState = POWER_STATE_UNKNOW;
    }
...
bool api_get_power_on (void)
{
    ...
    if (current_power_state == POWER_STATE_ON ||
        current_power_state == POWER_STATE_TRANSITIONING_TO_ON) {
        is_power_on = true;
    }
