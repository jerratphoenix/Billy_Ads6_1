...
static int32_t sensor_count =
    sizeof(sensor_callback_map) / sizeof(struct callback_map_t);

static struct sdr_rec_map* sdr_map_entry;

// All SDR raw bytes point
static uint8_t* sdr_raw_cache_ptr;
constexpr static const uint32_t sdr_raw_cache_ptr_size =
    MAX_SDR_RECORD_SIZE * MAX_SDR_MAP_ENTRIES;
...
constexpr const int defaultScanPeriod = 1000; //ms
 ...
 */
void assert_thresholds(struct sensor_var_t* sensor,
                       double assert_value,
                       LEVEL level,
                       DIRECTION direction,
                       bool assert)
{
    ...
    else if (level == LEVEL::WARNING && direction == DIRECTION::LOW)
    {
        property = "WarningAlarmLow";
        interface = sensor->warn_intf;
    }
    else if (level == LEVEL::CRITICAL && direction == DIRECTION::HIGH)
    {
        property = "CriticalAlarmHigh";
        interface = sensor->crit_intf;
    }
    else if (level == LEVEL::CRITICAL && direction == DIRECTION::LOW)
    {
 ...
 * @param   assert       Assert or deassert, true is assert
 */
void assert_discrete_offsets(struct sensor_var_t* sensor,
                             unsigned int offset,
                             bool assert)
{
    std::string property;
...
void check_discrete_offsets(struct sensor_var_t* sensor, double reading)
{
    ...
    for (offset = DISCRETE_OFFSET::OFFSET_0;
         offset <= DISCRETE_OFFSET::OFFSET_15;
         offset++)
    {
...
bool check_discrete_offset_retry(struct sensor_var_t* sensor, double reading)
{
         ...
         offset++)
    {
        ...
        if (sensor->have_assert_discrete_offset[offset] == true)
        {
            if (alarm == true)
            {
                if (sensor->assert_discrete_offset_retry_count[offset]++ <
                    sensor->have_assert_discrete_offset_retry[offset])
                {
                    DPRINT("%s offset %d assert retry count = %d\n",
                           sensor->name,
                           offset,
                           sensor->assert_discrete_offset_retry_count[offset]);
                    status = true;
                }
                else
                {
...
void check_thresholds(struct sensor_var_t* sensor, double reading)
{
    ...
    if (sensor->have_crit_low)
    {
        ...
        if (sensor->alarm_crit_low != alarm)
        {
            ...
            assert_thresholds(
                sensor, reading, LEVEL::CRITICAL, DIRECTION::LOW, alarm);
        }
    }
    ...
    if (sensor->have_crit_high)
    {
        ...
        if (sensor->alarm_crit_high != alarm)
        {
            ...
            assert_thresholds(
                sensor, reading, LEVEL::CRITICAL, DIRECTION::HIGH, alarm);
        }
    }
    ...
    if (sensor->have_warn_low)
    {
        ...
        if (sensor->alarm_warn_low != alarm)
        {
            ...
            assert_thresholds(
                sensor, reading, LEVEL::WARNING, DIRECTION::LOW, alarm);
        }
    }
    ...
    if (sensor->have_warn_high)
    {
        ...
        if (sensor->alarm_warn_high != alarm)
        {
            ...
            assert_thresholds(
                sensor, reading, LEVEL::WARNING, DIRECTION::HIGH, alarm);
        }
    }
 ...
 */
unsigned long int diff_timespec_ms(struct timespec end_ts,
...
bool is_sensor_power_on_delay(struct sensor_var_t* sensor)
{
    ...
    if (diff_timespec_ms(current_ts, get_last_poweron_ts()) <
...
bool is_sensor_power_off_delay(struct sensor_var_t* sensor)
{
    ...
    if (diff_timespec_ms(current_ts, get_last_poweroff_ts()) <
...
bool is_sensor_scan_enable(struct sensor_var_t* sensor)
{
    ...
    boost::asio::io_context io;
    auto conn = std::make_shared<sdbusplus::asio::connection>(io);
    auto method = conn->new_method_call(phoenix_sensor_service,
                                        sensor->obj_path,
...
int sensor_value_callback(struct sensor_var_t* sensor)
{
    ...
    if ((powergood == false &&
         sensor->reading_state == READING_STATE::POWERON) ||
...
int sensor_scan_service(void)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < sensor_count; i++)
        {
            ...
            // Check scan period
            if (diff_timespec_ms(sensor_scan_ts, sensor->last_scan_ts) <
                sensor->scan_period)
            {
                continue;
            }
            ...
            //DPRINT ( "%s callback use %ld ms\n", sensor->name, diff_timespec_ms(sensor->last_scan_ts, sensor_scan_ts));
 ...
 */
const char*
    get_sensor_unit_path(bool percentage, uint8_t sensor_type, uint8_t unit)
{
    const char* path;

    if (unit == UNIT_TYPE_CODE::DEGREES_C ||
        unit == UNIT_TYPE_CODE::DEFREES_F || unit == UNIT_TYPE_CODE::DEFREES_K)
    {
...
double sdr_convert_value(struct sdr_record_full_sensor* sensor, uint8_t val)
{
    ...
    {
        case 0:
            result = (double)(((m * val) + (b * std::pow(10, k1))) *
                              std::pow(10, k2));
            break;
        case 1:
            if (val & 0x80)
        ...
        /* fall through */
        case 2:
            result = (double)(((m * (int8_t)val) + (b * std::pow(10, k1))) *
                              std::pow(10, k2));
            break;
        default:
                    ...
                    int retry)
{
    ...
    method.append(
        "/", 0, std::array<const char*, 1>{item_board_intferface_name});
        ...
        for (int i = 0; i < 16; i++)
        {
            if (sensor->have_assert_discrete_offset[i] == true)
            {
                DPRINT("have_assert_discrete_offset[%d] = %s\n",
                       i,
                       (sensor->have_assert_discrete_offset[i] == true)
                           ? "true"
                           : "false");
            }
            if (sensor->have_deassert_discrete_offset[i] == true)
            {
                DPRINT("have_deassert_discrete_offset[%d] = %s\n",
                       i,
                       (sensor->have_deassert_discrete_offset[i] == true)
                           ? "true"
                           : "false");
            }
        }
...
int initial_sensor_capability_info(void)
{
    ...
    for (int index = 0; index < sensor_count; index++)
    {
        ...
        sensor_name_nospace.assign(sensor_callback_map[index].name,
                                   strlen(sensor_callback_map[index].name));
        sensor_name_nospace =
            boost::replace_all_copy(sensor_name_nospace, " ", "_");

        strcat(sensor_table[index].name, sensor_name_nospace.c_str());

        sensor_table[index].read_func = sensor_callback_map[index].read_func;

        sensor_table[index].reading_state =
            sensor_callback_map[index].reading_state;
        if (sensor_table[index].reading_state == READING_STATE::NOT_DEFINE)
        {
            sensor_table[index].reading_state = default_scan_reading_state;
        }

        sensor_table[index].scan_period =
            sensor_callback_map[index].scan_period;
        if (sensor_table[index].scan_period < 0)
        {
            sensor_table[index].scan_period = defaultScanPeriod;
        }

        sensor_table[index].power_on_delay =
            sensor_callback_map[index].power_on_delay;
        if (sensor_table[index].power_on_delay < 0)
        {
            sensor_table[index].power_on_delay = defaultPowerOnDelay;
        }

        sensor_table[index].power_off_delay =
            sensor_callback_map[index].power_off_delay;
        if (sensor_table[index].power_off_delay < 0)
        {
            sensor_table[index].power_off_delay = defaultPowerOffDelay;
        }

        sensor_table[index].have_crit_high_retry =
            sensor_callback_map[index].crit_high_retry;
        if (sensor_table[index].have_crit_high_retry < 0)
        {
            sensor_table[index].have_crit_high_retry = defaultThresholdRetry;
        }

        sensor_table[index].have_crit_low_retry =
            sensor_callback_map[index].crit_low_retry;
        if (sensor_table[index].have_crit_low_retry < 0)
        {
            sensor_table[index].have_crit_low_retry = defaultThresholdRetry;
        }

        sensor_table[index].have_warn_high_retry =
            sensor_callback_map[index].warn_high_retry;
        if (sensor_table[index].have_warn_high_retry < 0)
        {
            sensor_table[index].have_warn_high_retry = defaultThresholdRetry;
        }

        sensor_table[index].have_warn_low_retry =
            sensor_callback_map[index].warn_low_retry;
        if (sensor_table[index].have_warn_low_retry < 0)
        {
            sensor_table[index].have_warn_low_retry = defaultThresholdRetry;
        }

        sensor_table[index].have_assert_discrete_offset_retry[0] =
            sensor_callback_map[index].offset0_retry;
        sensor_table[index].have_assert_discrete_offset_retry[1] =
            sensor_callback_map[index].offset1_retry;
        sensor_table[index].have_assert_discrete_offset_retry[2] =
            sensor_callback_map[index].offset2_retry;
        sensor_table[index].have_assert_discrete_offset_retry[3] =
            sensor_callback_map[index].offset3_retry;
        sensor_table[index].have_assert_discrete_offset_retry[4] =
            sensor_callback_map[index].offset4_retry;
        sensor_table[index].have_assert_discrete_offset_retry[5] =
            sensor_callback_map[index].offset5_retry;
        sensor_table[index].have_assert_discrete_offset_retry[6] =
            sensor_callback_map[index].offset6_retry;
        sensor_table[index].have_assert_discrete_offset_retry[7] =
            sensor_callback_map[index].offset7_retry;
        sensor_table[index].have_assert_discrete_offset_retry[8] =
            sensor_callback_map[index].offset8_retry;
        sensor_table[index].have_assert_discrete_offset_retry[9] =
            sensor_callback_map[index].offset9_retry;
        sensor_table[index].have_assert_discrete_offset_retry[10] =
            sensor_callback_map[index].offset10_retry;
        sensor_table[index].have_assert_discrete_offset_retry[11] =
            sensor_callback_map[index].offset11_retry;
        sensor_table[index].have_assert_discrete_offset_retry[12] =
            sensor_callback_map[index].offset12_retry;
        sensor_table[index].have_assert_discrete_offset_retry[13] =
            sensor_callback_map[index].offset13_retry;
        sensor_table[index].have_assert_discrete_offset_retry[14] =
            sensor_callback_map[index].offset14_retry;
        sensor_table[index].have_assert_discrete_offset_retry[15] =
            sensor_callback_map[index].offset15_retry;

        for (int offset = 0; offset < 16; offset++)
        {
            if (sensor_table[index].have_assert_discrete_offset_retry[offset] <
                0)
            {
                sensor_table[index].have_assert_discrete_offset_retry[offset] =
        ...
        {
            if (strlen(sensor_callback_map[index].name) !=
                strlen(sdr_map_entry[i].sensor_name))
                continue;

            if (strcmp(sensor_callback_map[index].name,
                       sdr_map_entry[i].sensor_name) == 0)
            {
        ...
        if (sdrType == SDR_RECORD_TYPE_FULL_SENSOR)
        {
            ...
            strcat(sensor_table[index].obj_path, sensor_name_nospace.c_str());

            if (sdrData.event_type == 0x01)
            {
                is_discrete_sensor = false;

                sensor_table[index].have_crit_low =
                    sdrData.mask.type.threshold.assert_lcr_low;
                sensor_table[index].have_crit_high =
                    sdrData.mask.type.threshold.assert_ucr_high;

                sensor_table[index].have_warn_low =
                    sdrData.mask.type.threshold.assert_lnc_low;
                sensor_table[index].have_warn_high =
                    sdrData.mask.type.threshold.assert_unc_high;

                if (sensor_table[index].have_crit_low == true)
                {
                    sensor_table[index].crit_low = //static_cast<double>
                        (sdr_convert_value(&sdrData,
                                           sdrData.threshold.lower.critical));
                }
                ...
                if (sensor_table[index].have_crit_high == true)
                {
                    sensor_table[index].crit_high = //static_cast<double>
                        (sdr_convert_value(&sdrData,
                                           sdrData.threshold.upper.critical));
                }
                ...
                if (sensor_table[index].have_warn_low == true)
                {
                    sensor_table[index].warn_low = //static_cast<double>
                        (sdr_convert_value(
                            &sdrData, sdrData.threshold.lower.non_critical));
                }
                ...
                if (sensor_table[index].have_warn_high == true)
                {
                    sensor_table[index].warn_high = //static_cast<double>
                        (sdr_convert_value(
                            &sdrData, sdrData.threshold.upper.non_critical));
                }
                ...
                {
                    if (strcmp(sensor_table[index].unit,
                               sensor_unit_degreesc) == 0)
                    {
                        sensor_table[index].max_value = 127;
                        sensor_table[index].min_value = -128;
                    }
                    else if (strcmp(sensor_table[index].unit,
                                    sensor_unit_rpms) == 0)
                    {
                ...
                {
                    sensor_table[index].min_value =
                        (sdr_convert_value(&sdrData, sdrData.sensor_min));
                    sensor_table[index].max_value =
                        (sdr_convert_value(&sdrData, sdrData.sensor_max));
                }
            }
            else
            {
                ...
                for (int i = 0; i < 16; i++)
                {
                    sensor_table[index].have_assert_discrete_offset[i] =
                        ((sdrData.mask.type.discrete.assert_event & (1 << i)) >
                         0)
                            ? true
                            : false;
                    sensor_table[index].have_deassert_discrete_offset[i] =
                        ((sdrData.mask.type.discrete.deassert_event &
                          (1 << i)) > 0)
                            ? true
                            : false;
                }
        ...
        else
        {
            ...
            // set object path
            strcpy(sensor_table[index].obj_path, specific_sensor_path_prefix);
            strcat(sensor_table[index].obj_path, sensor_name_nospace.c_str());
...
void sensor_dbus_service(void)
{
    ...
    {
        std::shared_ptr<sdbusplus::asio::dbus_interface> item_board_interface =
            server.add_interface(base_board_path_prefix,
                                 item_board_intferface_name);

        item_board_interface->register_property(
            "Name",
    ...
    for (int index = 0; index < sensor_count; index++)
    {
        ...
        sensor_table[index].available_intf = server.add_interface(
            sensor_table[index].obj_path, available_interface_name);
        sensor_table[index].available_intf->register_property(
            "Available",
            (bool)false,
            sdbusplus::asio::PropertyPermission::readWrite);
        ...
        sensor_table[index].value_intf = server.add_interface(
            sensor_table[index].obj_path, sensor_value_interface_name);

        sensor_table[index].value_intf->register_property(
            "Value", (double)0, sdbusplus::asio::PropertyPermission::readWrite);
        ...
        */
        ...
        if (sensor_table[index].is_discrete_sensor == false)
        {
            ...
            {
                sensor_table[index].warn_intf =
                    server.add_interface(sensor_table[index].obj_path,
                                         sensor_warning_interface_name);
                if (sensor_table[index].have_warn_high == true)
                {
                    sensor_table[index].warn_intf->register_property(
                        "WarningHigh",
            ...
            {
                sensor_table[index].crit_intf =
                    server.add_interface(sensor_table[index].obj_path,
                                         sensor_critical_interface_name);
                if (sensor_table[index].have_crit_high == true)
                {
                    sensor_table[index].crit_intf->register_property(
                        "CriticalHigh",
        ...
        if (sensor_table[index].is_discrete_sensor == true)
        {
            sensor_table[index].discrete_offset_intf = server.add_interface(
                sensor_table[index].obj_path, sensor_discrete_interface_name);
            sensor_table[index].discrete_offset_intf->register_property(
                "AssertionAlarmOffset",
                (unsigned int)255,
                sdbusplus::asio::PropertyPermission::readWrite);
            sensor_table[index].discrete_offset_intf->register_property(
                "DeassertionAlarmOffset",
                (unsigned int)255,
                sdbusplus::asio::PropertyPermission::readWrite);
        ...
        if (board_path.length() > 0)
        {
            ...
            association_interface->register_property("Associations",
                                                     associations);
            association_interface->initialize();
        }

        // Operational interface
        std::shared_ptr<sdbusplus::asio::dbus_interface> operational_interface =
            server.add_interface(sensor_table[index].obj_path,
                                 operational_status_interface_name);
        operational_interface->register_property(
            "Functional",
            (bool)true,
            sdbusplus::asio::PropertyPermission::readWrite);
 ...
 * @return  status code
 */
static int mapping_sdr_rec(int sdr_repository_size, uint8_t* sdr_repository_ptr)
{
    long offset = 0;
    int sdrIndex = 0;
    ...
    while (offset < sdr_repository_size)
    {
        ...
        memcpy(&sdr_header,
               sdr_repository_ptr + offset,
               sizeof(struct sdr_rec_header));

        // Check Header invaild
        if (sdr_header.ver != ipmi_sdr_version || sdr_header.type == 0x00)
        {
            continue;
        }
        ...
        sdr_map_entry[sdrIndex].length =
            sizeof(struct sdr_rec_header) + sdr_header.length;
        ...
        switch (sdr_header.type)
        {
            case SDR_RECORD_TYPE_FULL_SENSOR:
                nameLength = *(sdr_repository_ptr + offset + 47) &
                             0x1f; //TODO: create struct, not hard code offset
                sdr_map_entry[sdrIndex].sensor_number =
                    *(sdr_repository_ptr + offset + 7);
                memcpy(sdr_map_entry[sdrIndex].sensor_name,
                       sdr_repository_ptr + offset + 48,
                       nameLength);
                break;
...
static int sdr_repository_load(void)
{
    ...
    // Allocate memory
    sdr_map_entry = (struct sdr_rec_map*)malloc(sizeof(struct sdr_rec_map) *
                                                MAX_SDR_MAP_ENTRIES);
    ...
    sdr_raw_cache_ptr =
        (uint8_t*)malloc(sizeof(uint8_t) * sdr_raw_cache_ptr_size);
